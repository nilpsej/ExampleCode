...
static long timeout = 0.5; //50ms timeout, in 10ms scaling
static long hold = 0.1; //10ms hold, in 10ms scaling
static long loop = 0.04; //4ms loop rate, in 10ms scaling
	...
	if (mask || value) {
		for (i=0; i < 2048 ;i++) {
			if ((i & mask) ==  (value & mask))
				do_set(i, ENABLE); //find out what this line does...
			else
				do_clr(i, ENABLE); //ELSE, REMOVE ID FROM MONITORING? double check
		}
	}
*/
/*
	if (quiet) //IF QUIET IS TRUE, ALL IDS ARE DEACTIVATED. 
		for (i=0; i < 2048 ;i++)
			do_clr(i, ENABLE);
*/
/*
...
//define interface name...check where this is used..
	interface = "vcan0";
//setup socket..
...
//	printf("%s", CSR_HIDE); /* hide cursor */
	...
	while (running) {
		...
		//printf("current cms: %ld, usec: %ld.\n", currcms, tv.tv_usec);
		//currcms = (tv.tv_sec - start_tv.tv_sec) + (tv.tv_usec);
/*
		if (FD_ISSET(0, &rdfs))
